...
public class CreateLabelIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired DistributorQueryApi distributorQueryService;

    @Test
    void createLabel_persistsLabelWithAllFields() {
        var user = createTestUser("create-test@example.com");
        var address = new Address("123 Main St", "Suite 100", "Oslo", "0123", "Norway");
    ...
    void createLabel_createsDefaultDirectDistributor() {
        ...
        var label =
                labelCommandApi.createLabel(
                        "Label With Default Channel", null, null, null, null, user.getId());

        var distributors = distributorQueryService.findByLabelId(label.id());
        assertThat(distributors).hasSize(1);
        assertThat(distributors.getFirst().name()).isEqualTo("Direct Sales");
        assertThat(distributors.getFirst().channelType()).isEqualTo(ChannelType.DIRECT);
    }
