...
public interface SaleRepository extends JpaRepository<SaleEntity, Long> {
     ...
     *
     * <p>Joins through {@code inventory_movement} (where {@code movement_type = 'SALE'} and {@code
     * reference_id} is the sale ID), so results are scoped to the exact production run — not to all
     * runs sharing the same {@code release_id + format}. This is correct even when a release has
     * multiple production runs (repressings).
     */
    @Query(
            value =
                    """
    ...
    @Query(
            "SELECT COALESCE(SUM(s.totalAmount), 0) FROM SaleEntity s "
